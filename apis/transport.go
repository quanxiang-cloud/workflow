...
func NewHTTPHandler(conf *common.Config, s Service, logger log.Logger) http.Handler {
	r := gin.Default()
...

	{
		...
		group.POST("/pipeline", gin.WrapH(httptransport.NewServer(
			e.PostSavePipelineEndpoint,
			func(ctx context.Context, r *http.Request) (request interface{}, err error) {
				var req SavePipeline
		...
		group.POST("/pipeline/:name/exec", func(c *gin.Context) {
			name := c.Param("name")
			httptransport.NewServer(
				e.PostExecPipelineEndpoint,
				func(ctx context.Context, r *http.Request) (request interface{}, err error) {
					var req ExecPipeline
					req.Name = name
					return reqJSON(&req)(ctx, r)
				},
				responseJSON,
		...
		group.POST("/pipelineRun/:id/exec", func(c *gin.Context) {
			id := c.Param("id")
			httptransport.NewServer(
				e.PostExecpipelineRunEndpoint,
				func(ctx context.Context, r *http.Request) (request interface{}, err error) {
